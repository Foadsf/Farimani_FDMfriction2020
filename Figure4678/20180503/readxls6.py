...
dfs['min']=dfs.iloc[argrelextrema(dfs.iloc[0:,2].values, np.less, order=n)[0],2]
dfs['max']=dfs.iloc[argrelextrema(dfs.iloc[0:,2].values, np.greater, order=n)[0],2]
# print(argrelextrema(dfs.iloc[0:,2].values, np.less, order=10)[0])
# print(dfs.iloc[:,2].values)
...
# dfs['diff']=np.insert(np.diff(np.sign(np.diff(dfs.force))),0,[0,0])
# print(dfs.shape)

import matplotlib.pyplot as plt
plt.plot(dfs.iloc[:,1], dfs.iloc[:,2],label='data')
plt.scatter(dfs2.iloc[:,1], dfs2.iloc[:,4],label='min')
plt.scatter(dfs3.iloc[:,1], dfs3.iloc[:,5],label='max')
